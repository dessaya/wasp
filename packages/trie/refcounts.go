...
type Refcounts struct {
	nodes  *kvStorePartition
	values *kvStorePartition
}

func newRefcounts(store KVStore) *Refcounts {
	return &Refcounts{
		nodes:  (*kvStorePartition)(makeKVStorePartition(store, partitionRefcountNodes)),
		values: (*kvStorePartition)(makeKVStorePartition(store, partitionRefcountValues)),
	}
}
...
func (d *Refcounts) Inc(node *bufferedNode) {
	...
	node.nodeData.iterateChildren(func(i byte, commitment Hash) bool {
		if _, ok := node.uncommittedChildren[i]; !ok {
			// add a reference to an "old" node
...
func incRefcount(s *kvStorePartition, key []byte) uint32 {
	n := getRefcount(s, key)
	n++
	setRefcount(s, key, n)
	return n
...
func decRefcount(s *kvStorePartition, key []byte) uint32 {
	n := getRefcount(s, key)
	if n == 0 {
...
func getRefcount(s *kvStorePartition, key []byte) uint32 {
	b := s.Get(key)
	if b == nil {
		return 0
	}
...
func setRefcount(s *kvStorePartition, key []byte, n uint32) {
	if n == 0 {
		s.Del(key)
	} else {
