...
func initializedStore(db kvstore.KVStore) state.Store {
	...
	origin.InitChain(migrations.LatestSchemaVersion, st, initArgs, iotago.ObjectID{}, 0, parameterstest.L1Mock)
	return st
}
...
func TestOriginBlockDeterminism(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		blockA, _ := origin.InitChain(migrations.LatestSchemaVersion, st, initArgs, iotago.ObjectID{}, deposit, parameterstest.L1Mock)
		blockB, _ := origin.InitChain(migrations.LatestSchemaVersion, st, initArgs, iotago.ObjectID{}, deposit, parameterstest.L1Mock)
		...
		blockC, _ := origin.InitChain(migrations.LatestSchemaVersion, st2, initArgs, iotago.ObjectID{}, deposit, parameterstest.L1Mock)
...
func TestEqualStates(t *testing.T) {
	...
	time1 := time.Now()
	draft1 := cs1.NewStateDraft(time1, origin.L1Commitment(migrations.LatestSchemaVersion, initArgs, iotago.ObjectID{}, 0, parameterstest.L1Mock))
	...
	db2 := mapdb.NewMapDB()
	cs2 := mustChainStore{initializedStore(db2)}
	draft1 = cs2.NewStateDraft(time1, origin.L1Commitment(migrations.LatestSchemaVersion, initArgs, iotago.ObjectID{}, 0, parameterstest.L1Mock))
	draft1.Set("b", []byte("variable b"))
	draft1.Set("a", []byte("variable a"))
...
func TestDiffStatesTimestamp(t *testing.T) {
	db1 := mapdb.NewMapDB()
	cs1 := mustChainStore{initializedStore(db1)}
	draft1 := cs1.NewStateDraft(time.Now(), origin.L1Commitment(migrations.LatestSchemaVersion, initArgs, iotago.ObjectID{}, 0, parameterstest.L1Mock))
	draft1.Set("a", []byte("variable a"))
	block1 := cs1.Commit(draft1)
	state1 := cs1.StateByTrieRoot(block1.TrieRoot())
	...
	db2 := mapdb.NewMapDB()
	cs2 := mustChainStore{initializedStore(db2)}
	draft1 = cs2.NewStateDraft(time.Now(), origin.L1Commitment(migrations.LatestSchemaVersion, initArgs, iotago.ObjectID{}, 0, parameterstest.L1Mock))
	draft1.Set("a", []byte("variable a"))
	block1 = cs2.Commit(draft1)
	state2 := cs2.StateByTrieRoot(block1.TrieRoot())
